...
class DrGRPO(RLAlgorithm):
    ...
    def update(
        ...
        clip_high = 1.0 + cfg.clip_eps + max(0.0, cfg.clip_eps_high)
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                adv = mb.advantages.clamp(-cfg.adv_clip, cfg.adv_clip).detach()[:, None]
                new_logp = compute_per_token_logprobs(model, mb.input_ids, mb.attention_mask)

                ratio = torch.exp(new_logp - mb.old_logprobs)
                clipped_ratio = ratio.clamp(min=clip_low, max=clip_high)

                unclipped_obj = ratio * adv
                clipped_obj = clipped_ratio * adv
                token_obj = torch.minimum(unclipped_obj, clipped_obj)
                # seq_obj = masked_mean_per_row(token_obj, mask)
                seq_obj = (token_obj * mask).sum(dim=1)
                ...
                clipped_mask = ((ratio < clip_low) | (ratio > clip_high)).float() * mask
                ...
                total_clip_frac += float(clipped_mask.sum().item() / mask.sum().item())
                n_mb += 1
        ...
        return {
            ...
            "train/ppo_clipped_token_fraction_mean_over_minibatches": total_clip_frac / denom,
