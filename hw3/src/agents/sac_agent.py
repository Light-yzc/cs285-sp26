...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qs = torch.stack([critic(next_obs, next_action) for critic in self.target_critics], dim=0)
            # ENDTODO
        ...
        # Predict Q-values
        q_values = torch.stack([single_critic(obs, action) for single_critic in self.critics])
